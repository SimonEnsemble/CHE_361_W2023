...
*  $\alpha$ [g solution /g salt]: describes dependence of solution density on salt concentration
...
*  $\alpha=0.64$ g / g                                                              
*  $\rho_w=1000$ g/L 
also, the water input is constant:
*  $q=0.1$ L/s     

!!! note \"task\"
	our task then in this studio is to predict how the output $c(t)$ behaves in response to the input $w(t)$. the other input, $q(t)$, is constant.
	
🐝 define `V`, `α`, `ρw`, and `q` as variables in Julia in the cell below. we shall use these variables in the rest of the code.
"  
...
!!! example \"customer standards\"
	the customer (a hospital) wishes for saline solution with a salt concentration of $\bar{c}=0.1$ g/L. however, they will accept (buy) saline solution with salt concentration $c\in[0.095, 0.105]$ g/L. ie., the customer standards are for the saline solution to have a salt concentration within 0.005 g/L of the target $\bar{c}$.
                                                                                            

🐝 to produce saline solution that meets the customer specifications, we wish to achieve and maintain an output salt concentration of $\bar{c}$ at steady state conditions. given the value of the constant incoming flow rate of water, $q$, compute the corresponding steady state value $\bar{w}$ [g/s] of the incoming salt flow rate, $w$, and assign it as a variable `w̄`.
                                                                                    
!!! hint
    you probably need to use `fzero`. see `Roots.jl` docs [here](https://github.com/JuliaMath/Roots.jl#usage-examples).                                                
" 
...
suppose the process is initially ($t=0$) operating at steady state: (i) with inlet salt flow rate $\bar{w}$ and (ii) producing saline solution with salt concentration $\bar{c}$. this implies that the salt concentration in the solution in the tank is initially $\bar{c}$ as well.

these steady state conditions are maintained until $t=25$ s.

at $t=25$ s, the conveyor belt loses power and shuts off. 
only at $t=100$ s does the engineer on staff address the problem and switch the conveyor belt back on to its normal speed to (immediately) achieve $\bar{w}$ again for the input salt concentration.
...
md"🐝 to verify your `w(t)` function, use it to plot the rectangular pulse input $w(t)$ over the domain $t\in[0,10]$ min (**check units**). include x- and y-axis labels and a title \"rectangular pulse function\".
"
...
md"🐝 before you simulate, sketch out what you anticipate the output salt concentration $c(t)$ to look like in response to this rectangular pulse input $w(t)$. this should test your intuition of process dynamics.

🚀 **simulation time**! 🚀

🐝 use `DifferentialEquations.jl` to simulate $c(t)$, the concentration of salt in the outlet stream of the process, for $t\in[0, 10]$ min (**check units**). 
* plot the numerical solution, $c(t)$, for $t\in[0, 10]$ min.
...
!!! hint
	you will see an unphysical blip in the solution, owing to a discontinuity in the input. to alleviate this numerical artifact, pass `d_discontinuities=[25.0, 100.0]` to `ODEProblem` to increase the resolution around the discontinuities.
"
...
```
to highlight, on the same plot, the time interval over which the saline solution cannot be sold. see the [`CairoMakie.jl` docs](https://docs.makie.org/v0.19/examples/plotting_functions/hvspan/index.html#vspan).
"
