...
```math
  \begin{align}
      V_g \frac{dg}{dt} &= i - \beta g V_g \\
      V_b\frac{db}{dt} &= \beta g V_g - \alpha b V_b
  \end{align}
```
...
!!! hint
	use `vspan!` [here](https://makie.juliaplots.org/stable/examples/plotting_functions/hvspan/) and apply this to the figure returned from `viz_response` via [here](https://simonensemble.github.io/Controlz.jl/dev/viz/#modifying-the-figures).
"
